...
def main():
    ...
    # Make a list of images in dataset
    img_paths = []
    for root, dirs, files in os.walk(TRAIN_IMAGE_PATH):
        for _ in files :
            img_paths.append(root + _)
    ...
    with tf.Graph().as_default():
        ...
        gen_image = model.generator(image, training=True)
        squeezed_gen_image = tf.image.encode_jpeg(tf.cast(tf.squeeze(gen_image, [0]), tf.uint8))

        # Obtain loss model layers for image, gen_image and style image.
        _, endpoints_mixed = model.loss_model(tf.concat([image, gen_image, style_image], 0))
        variables_to_restore = slim.get_variables_to_restore(include=[LOSS_MODEL])
        ...
        # tf.summary.image('generated', gen_image)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(EPOCH_NUM):
                ...
                for i in range(DATA_SIZE // BATCH_SIZE):
                    image_batch, batch_index = data_process.read_batches(TRAIN_IMAGE_PATH, img_paths, batch_index, BATCH_SIZE)
                    ...
                    if i % 10 == 0:
                        ...
                        styled_image = sess.run(squeezed_gen_image, feed_dict={image: test_image})
